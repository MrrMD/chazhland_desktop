...
export function ConfirmModal({ title, message, confirmLabel, danger, onConfirm, onClose }: {
  ...
  return (
    <Modal title={title} onClose={onClose}>
      <div style={{ fontSize: 14, color: 'var(--text-2)', lineHeight: 1.6, marginBottom: 20 }}>{message}</div>
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        <button className="pill no-drag" onClick={onClose} style={{ padding: '10px 16px', fontWeight: 600 }}>Отмена</button>
        <button className={(danger ? 'danger-btn' : 'accent-btn') + ' no-drag'} onClick={onConfirm} style={{ borderRadius: 12, padding: '10px 18px', fontWeight: 700 }}>{confirmLabel}</button>
      </div>
    </Modal>
  )
}
...
export function ChangeRoleModal({ member, onSelect, onClose }: { member: Member; onSelect: (r: Role) => void; onClose: () => void }) {
  const [sel, setSel] = useState<Role>(member.role)
  return (
    <Modal title={`Роль · ${member.username}`} onClose={onClose}>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        <button className="pill no-drag" onClick={onClose} style={{ padding: '10px 16px', fontWeight: 600 }}>Отмена</button>
        <button className="accent-btn no-drag" disabled={sel === member.role} onClick={() => onSelect(sel)} style={{ borderRadius: 12, padding: '10px 18px', fontWeight: 700, opacity: sel === member.role ? 0.5 : 1 }}>Сохранить</button>
      </div>
    </Modal>
  )
}
