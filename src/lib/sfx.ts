...
class Sfx {
  ...
  // ---- уведомления (вне голоса) ----
  mention() { this.play([988, 1319, 1568], 0.08) }   // упоминание/ответ тебе — яркое восходящее трезвучие
  dm() { this.play([784, 1047], 0.085) }             // личное сообщение — мягкий двойной «пинг»
  reaction() { this.play([1318], 0.06) }             // реакция на твоё сообщение — короткий высокий «тук»
  error() { this.play([300, 200], 0.1, 'triangle') } // ошибка — низкое нисходящее
  disconnect() { this.play([520, 300], 0.11) }       // связь потеряна — вниз
  reconnect() { this.play([400, 620], 0.09) }        // связь восстановлена — вверх
  watchStart() { this.play([523, 659, 784], 0.09) }  // совместный просмотр запущен — мажорное «showtime»
}
